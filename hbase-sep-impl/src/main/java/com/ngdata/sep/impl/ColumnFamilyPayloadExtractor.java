 ...
 * mutation.
 */
public class ColumnFamilyPayloadExtractor implements PayloadExtractor {
     ...
     */
    public ColumnFamilyPayloadExtractor(byte[] tableName, byte[] columnFamily) {
    ...
    public byte[] extractPayload(byte[] tableName, KeyValue keyValue) {
        if (Bytes.equals(this.tableName, tableName) && CellUtil.matchingFamily(keyValue, columnFamily)) {
